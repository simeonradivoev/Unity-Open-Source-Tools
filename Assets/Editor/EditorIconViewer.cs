...
public class EditorIconViewer : EditorWindow {
	...
	{
		EditorIconViewer w = (EditorIconViewer)EditorWindow.GetWindow<EditorIconViewer>();
		w.Show();
	}
	...
	void OnGUI()
	{
		...
		if (Drawings == null)
		{
			...
			else if (_showingIcons)
			{
				if (_objects == null)
				{
					_objects = new List<UnityEngine.Object>(Resources.FindObjectsOfTypeAll(typeof(Texture2D)).Where(t => AssetDatabase.GetAssetPath(t).StartsWith("Library")));
